...
const getWeather = async (location) => {
  ...
  const { latitude, longitude, name } = geocodingData.results[0];
  const weatherUrl = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,wind_gusts_10m,weather_code,pressure_msl,precipitation,cloud_cover,uv_index,visibility,is_day,dew_point_2m`;
  const response = await fetch(weatherUrl);
  const data = await response.json();
  ...
  return {
    ...
    isDay: c.is_day === 1,
    conditions: getWeatherCondition(c.weather_code),
    location: name
  };
};
...
async function startAsyncWorkflowHandler$1({
  ...
  try {
    ...
    if (payloadRuntimeContext) {
      runtimeContext = new RuntimeContext$1();
